...
type LinkingsStore = {
  ...
  acceptLinking: (linkingId: number) => Promise<void>;
  rejectLinking: (linkingId: number) => Promise<void>;
  stopLinking: (linkingId: number) => Promise<void>;
};
...
export const useLinkingsStore = create<LinkingsStore>((set, get) => ({
  ...
  fetchLinkings: async () => {
    ...
    try {
      ...
      set({ linkings: testInitialLinkings, loading: false });
      return;
      ...
      const linkings = Array.isArray(data) ? data : [];
  ...
  acceptLinking: async (linkingId: number) => {
    set({ loading: true, error: null });
    try {
      const accessToken = useAuthStore.getState().accessToken;
      const response = await fetch(`${API_BASE}/linkings/${linkingId}/accept`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${accessToken}`,
        },
  ...
  rejectLinking: async (linkingId: number) => {
    set({ loading: true, error: null });
    try {
      const accessToken = useAuthStore.getState().accessToken;
      const response = await fetch(`${API_BASE}/linkings/${linkingId}/reject`, {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${accessToken}`,
        },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || `Failed to reject linking: ${response.status}`);
      }
  ...
  stopLinking: async (linkingId: number) => {
    set({ loading: true, error: null });
    try {
      const accessToken = useAuthStore.getState().accessToken;
      const response = await fetch(`${API_BASE}/linkings/${linkingId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${accessToken}`,
        },
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.detail || `Failed to stop linking: ${response.status}`);
      }
